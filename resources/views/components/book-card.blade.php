        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-4 md:p-5 text-center">
                ...
                <ul class="flex flex-wrap text-sm font-medium text-center text-gray-500 border-b border-gray-200 rounded-t-lg bg-gray-50 dark:border-gray-700 dark:text-gray-400 dark:bg-gray-800"
                    ...
                    <li class="me-2">
                        <button id="about-tab" data-tabs-target="{{ '#about_' . $bookId }}" type="button"
                    ...
                    <li class="me-2">
                        <button id="description-tab" data-tabs-target="{{ '#description_' . $bookId }}" type="button"
                            role="tab" aria-controls="description" aria-selected="false"
                            class="inline-block p-4 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 dark:hover:text-gray-300">Description</button>
                    </li>
                    <li class="me-2">
                        <button id="shop-tab" data-tabs-target="{{ '#shop_' . $bookId }}" type="button"
                            role="tab" aria-controls="statistics" aria-selected="false"
                            class="inline-block p-4 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700 dark:hover:text-gray-300">Shop</button>
                    </li>
                </ul>
                <div id="defaultTabContent">
                    <div class="hidden p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800" id="{{ 'about_' . $bookId }}"
                        role="tabpanel" aria-labelledby="about-tab">
                        <h2 class="mb-3 text-3xl font-extrabold tracking-tight text-gray-900 dark:text-white">
                            {{ $title }}</h2>
                        <span class="mb-3 text-gray-500 dark:text-gray-400">{{ $searchInfo }}</span>
                    </div>
                    <div class="hidden p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800"
                        id="{{ 'description_' . $bookId }}" role="tabpanel" aria-labelledby="categories-tab">
                    ...
                    <div class="hidden p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800" id="{{ 'shop_' . $bookId }}"
                        role="tabpanel" aria-labelledby="statistics-tab">
                        <dl class="grid max-w-screen-xl gap-8 p-4 mx-auto text-gray-900 dark:text-white sm:p-8">
                            <div class="flex flex-col">
                                <dt class="mb-2 text-3xl font-extrabold {{ !empty($shop) ? '' : 'line-through' }}">
                                    {{ !empty($saleInfo) ? numfmt_format_currency(numfmt_create('pt_BR', NumberFormatter::CURRENCY), floatval($saleInfo), $currencyCode) : (!empty($shop) ? "FREE" : "NOT AVAILABLE") }}
                                </dt>
                                <button onclick="{{ !empty($shop) ? "window.open('$shop')" : 'disabled' }}"
                                    class="block text-white bg-[#6875f5] hover:bg-[#4f5ef0] font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-[#4f5ef0] dark:hover:bg-[#4f5ef0]"
